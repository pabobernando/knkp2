...
function DataSarpras() {
    ...
    const [selectedSarpras, setSelectedSarpras] = useState()
    const [page, setPage] = useState(0)
    const [halaman, setHalaman] = useState()
    const [pageCount, setPageCount] = useState(0)

    const openModal = (sarpras) => {
      console.log(sarpras);
      setSelectedSarpras(sarpras)
      setIsOpen(true);
    };

    const getDataSarpras = ({ page = 0, limit = 10, cabor_id = '' }) => {
      const token = localStorage.getItem('token');
      const headers = {
        Authorization: `Bearer ${token}`
      };
      fetch(`http://103.193.176.29:3000/api/v1/sarpras?${new URLSearchParams({
        page,
        limit,
        cabor_id,
      }).toString()}`,
    ...
    const handleDelete = () => {
      ...
      fetch(`http://103.193.176.29:3000/api/v1/sarpras/${sarpras.id}`, {
        method: 'DELETE',
        headers: {
    ...
    const handleAdd = (form) => {
      const token = localStorage.getItem('token');
           ...
           fetch('http://103.193.176.29:3000/api/v1/sarpras', {
             method: 'POST',
             headers: {
               'Content-Type': 'application/json',
               ...headers
    ...
    const downloadFile = async () => {
      ...
      try {
        const response = await fetch(
          "http://103.193.176.29:3000:3000/api/v1/sarpras/download",
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
...
<main className="relative h-screen overflow-hidden bg-gray-100 dark:bg-gray-800">
    <ModalDelete onClose={closeModal} onOk={handleDelete} isOpen={isOpen} dataName={selectedSarpras?.nama} />
    <ModalAdd onClose={closeModalAdd} onOk={handleOk} isOpen={isOpenAdd} initialValue={selectedSarpras} />
    <div className="flex items-start justify-between">
        ...
        <div className="flex flex-col w-full md:space-y-4">
            ...
            <div className="h-screen px-4 pb-24 overflow-auto md:px-6">
                ...
                <DropdownCabor onChange={(idCabor) =>  getDataSarpras({ cabor_id: idCabor })} />
...
<button onClick={openModalAdd} className="bg-blue-500 hover:bg-blue-700 text-gray-800 font-bold py-1 px-3 rounded inline-flex items-center">
  ...
  <span className="text-white hidden sm:flex">Tambah Data Sarpras</span>
</button>
                    </div>
                </div>
            </header>
        ...
        <tbody className='bg-white divide-y divide-gray-200'>
          {sarprass.map((sarpras, index) => (
            <tr key={index}>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{sarpras.nama}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{sarpras.keterangan}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{sarpras.tahun}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{sarpras.cabor.nama}</td>
